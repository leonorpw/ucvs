<?php
	...
	if($check != true)
	{
		cLog::Log("Wrong IP requested UCVS! - IP: " . $reqIP);
		die('Wrong IP!');
	}
	else 
	{
		...
		if($result !== true)
		{
			cLog::Log("Error while processing data: " . $result . " - User ID: " . $ucvs->getUser($the_request) . " Site: " . $ucvs->getSite($reqIP));
			echo $result;
		}
		else
		{
			cLog::Log("Valid UCVS request finished successfull" . " - User ID: " . $ucvs->getUser($the_request) . " Site: " . $ucvs->getSite($reqIP));
			echo 'OK';
		}
	}
?>